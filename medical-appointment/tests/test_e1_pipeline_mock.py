    ...
    def predict(self, premises, questions):
        ...
        for premise, question in zip(
            ...
            if "200 mg" in q and "100 mg" in p:
                # Intentionally wrong: deterministic fact guard must fix it.
                results.append(
                    NLIResult(0.80, 0.10, 0.10)
                )
            elif (
                "500 mg" in q
                and (
                    "500 mg" in p
                    or "five hundred" in p
                )
            ):
                results.append(
                    NLIResult(0.82, 0.14, 0.04)
                )
            elif (
                "500 mg" in q
                and (
...
class PipelineMockTests(unittest.TestCase):
    def test_positive_dual_asr(self):
        pipeline = DualASRE1Pipeline(
            ...
            device="cpu",
            medasr=FakeASR(
                "the doctor prescribed metformin 500 mg twice daily with meals"
            ),
            parakeet=FakeASR(
                "the doctor prescribed metformin 500 mg twice daily with meals"
            ),
    ...
    def test_hard_negative_even_when_nli_is_fooled(self):
        pipeline = DualASRE1Pipeline(
            ...
            device="cpu",
            medasr=FakeASR(
                "the dose was 100 mg once daily"
            ),
            parakeet=FakeASR(
                "the dose was 100 mg once daily"
            ),
        ...
        out = pipeline.predict_audio(
            [0.0],
            [
                "Was the dose 200 mg once daily?"
            ],
        )
    ...
    def test_asr_disagreement_keeps_exact_fact_yes(self):
        pipeline = DualASRE1Pipeline(
            ...
            device="cpu",
            medasr=FakeASR(
                "start metformin 50 mg twice daily"
            ),
            parakeet=FakeASR(
    ...
    def test_single_strong_source_plus_neutral_can_answer_yes(self):
        pipeline = DualASRE1Pipeline(
            config=E1Config(
                retrieval_window_words=16,
                retrieval_secondary_window_words=8,
            ...
            device="cpu",
            medasr=FakeASR(
                "the patient clearly reports nausea today"
            ),
            parakeet=FakeASR(
                "patient mentions feeling unwell today"
            ),
            nli=FakeNLI(),
        )
        out = pipeline.predict_audio(
