...
from e1.evidence import (
    build_evidence_proposals,
    select_refined_evidence,
)
from e1.facts import compare_facts, extract_facts
from e1.nli import question_to_hypothesis
from e1.pairing import candidates_align, pair_assessments
...
class E1LogicTests(unittest.TestCase):
    ...
    def test_number_and_unit_normalization(self):
        self.assertIn(
            "500 mg",
            normalize_text("five hundred milligrams"),
        )
        self.assertIn(
            "14 day",
            normalize_text("fourteen days"),
        )
    ...
    def test_declarative_hypothesis(self):
        self.assertEqual(
            question_to_hypothesis(
                "Did the patient report chest pain?"
            ),
            "The patient did report chest pain.",
        )
        self.assertEqual(
            question_to_hypothesis(
                "Was the patient prescribed metformin?"
            ),
    ...
    def test_retrieval_does_not_require_wrong_number_to_match(self):
        words = [
            ...
            for i, token in enumerate(
                (
                    "we discussed exercise then prescribed metformin "
                    "five hundred milligrams twice daily for diabetes "
                    "follow up"
                ).split()
            )
        ]
    ...
    def test_temporal_alignment(self):
        ...
        self.assertTrue(
            candidates_align(a, b, self.config)
        )
    ...
    def test_fact_guard_overrides_false_entailment(self):
        ...
        self.assertGreater(
            assessment.no_score,
            assessment.yes_score,
        )
        self.assertTrue(
            assessment.facts.has_strict_contradiction
        )

    def test_paired_event_drives_dual_yes(self):
        ...
        assessments = [
            assess_candidate(
                q,
                med,
                NLIResult(0.72, 0.24, 0.04),
                self.config,
            ),
            assess_candidate(
                q,
                par,
                NLIResult(0.70, 0.26, 0.04),
                self.config,
            ),
        ]
        events = pair_assessments(
            [assessments[0]],
            [assessments[1]],
            self.config,
        )
    ...
    def test_unrelated_contradiction_does_not_veto_yes_event(self):
        q = "Did the patient report nausea?"

        med_yes = CandidateWindow(
            "medasr",
            0,
            6,
            10.0,
            12.0,
            "the patient reported nausea yesterday",
            "the patient reported nausea yesterday",
            retrieval_score=1.0,
            topic_overlap=1.0,
        )
        par_yes = CandidateWindow(
            "parakeet_v3",
            0,
            6,
            10.1,
            12.1,
            "patient reports nausea yesterday",
            "patient reports nausea yesterday",
            retrieval_score=1.0,
            topic_overlap=1.0,
        )
        med_no = CandidateWindow(
            "medasr",
            20,
            28,
            70.0,
            73.0,
            "no vomiting was reported at follow up",
            "no vomiting was reported at follow up",
            retrieval_score=0.8,
            topic_overlap=0.2,
        )
        ...
        assessments = [
            assess_candidate(
                q,
                med_yes,
                NLIResult(0.78, 0.18, 0.04),
                self.config,
            ),
            assess_candidate(
                q,
                par_yes,
                NLIResult(0.76, 0.20, 0.04),
                self.config,
            ),
            assess_candidate(
                q,
                med_no,
                NLIResult(0.08, 0.12, 0.80),
                self.config,
            ),
        ]
        events = pair_assessments(
            [assessments[0], assessments[2]],
            [assessments[1]],
            self.config,
        )
        decision = decide(
    ...
    def test_compact_evidence_refinement(self):
        text = (
            "today we discussed diet and then start metformin "
            "500 mg twice daily with meals and review in four weeks"
        )
        pieces = text.split()
        words = [
            NormalizedWord(
        ...
        nli = [
            NLIResult(
                0.85 if "500" in p.text else 0.50,
                0.10,
                0.05,
            )
            for p in proposals
        ]
        span = select_refined_evidence(
            proposals,
            nli,
            {"medasr": transcript},
            self.config,
        ...
        self.assertLessEqual(
            span.end_word - span.start_word + 1,
            self.config.evidence_max_words,
        )
