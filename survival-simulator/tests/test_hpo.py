...
def test_case_names_continue_after_v2_study():
    assert case_id_for_trial_number(0) == "C402"
    assert case_id_for_trial_number(99) == "C501"


def test_focused_search_only_tunes_declared_v3_subset():
    expected = {
...
def test_c376_v3_base_is_valid_and_inside_every_search_interval():
    base = _base()
    validate_focused_base_config(base)
    assert base.architecture_v2_enabled is True
    assert base.architecture_v3_enabled is True
    assert base.planner_enabled is True
    assert base.planner_horizon_steps == 3
    assert base.planner_beam_width == 1
    for name, spec in SEARCH_SPACE.items():
        value = getattr(base, name)
        assert spec.low <= value <= spec.high, name


def test_v2_only_base_is_rejected_for_focused_hpo():
    values = _base().to_dict()
    values["architecture_v3_enabled"] = False
    with pytest.raises(ValueError, match="architecture_v3_enabled"):
...
def test_fixed_trial_builds_valid_config_and_preserves_frozen_fields():
    ...
    sampled = {}
    for name, spec in SEARCH_SPACE.items():
        sampled[name] = int((int(spec.low) + int(spec.high)) // 2) if spec.kind == "int" else (float(spec.low) + float(spec.high)) / 2.0
...
def test_robust_objective_uses_mean_and_p10():
    summary = {
        ...
        "successful_runs": 30,
        "failed_runs": 0,
        "aggregate": {"score": {"mean": 100.0, "median": 95.0, "min": 20.0, "max": 180.0, "p10": 60.0}},
    }
...
def test_partial_batch_is_not_scored():
    summary = {"n_runs": 30, "successful_runs": 29, "failed_runs": 1, "aggregate": {"score": {"mean": 100.0, "p10": 60.0}}}
...
def test_hpo_orchestration_enqueues_exact_base_then_samples(monkeypatch, tmp_path):
    ...
    def fake_run_candidate(*, base_seed, config, output_dir, n_cores):
        ...
        summary = {
            "status": "success", "base_seed": base_seed, "n_cores": n_cores,
            "n_runs": n_cores, "successful_runs": n_cores, "failed_runs": 0,
            "aggregate": {"score": {"mean": mean, "median": mean - 1.0, "min": mean - 10.0, "max": mean + 10.0, "p10": mean - 5.0}},
            "runs": [],
        }
    ...
    study = hpo.run_hpo(
        results_root=tmp_path, base_config=base, base_seed=90000, n_cores=2,
        n_trials=3, sampler_seed=123, startup_trials=1,
        objective_mode="robust", study_name="test_focused_hpo_v3",
    )
    assert len(study.trials) == 3
    for case in ("C402", "C403", "C404"):
        assert (tmp_path / case / "hpo_trial.json").exists()
    assert (tmp_path / "hpo_v3_focused_study.db").exists()
    ...
    c402 = PolicyConfig.from_json(tmp_path / "C402" / "params.json")
    assert c402.to_dict() == base.to_dict()
    meta = json.loads((tmp_path / "C402" / "hpo_trial.json").read_text())
...
def test_resume_rejects_changed_seed_batch(monkeypatch, tmp_path):
    ...
    def fake_run_candidate(*, base_seed, config, output_dir, n_cores):
        ...
        summary = {
            "n_runs": n_cores, "successful_runs": n_cores, "failed_runs": 0,
            "aggregate": {"score": {"mean": 100.0, "median": 100.0, "min": 100.0, "max": 100.0, "p10": 100.0}},
        }
        (output_dir / "summary.json").write_text(json.dumps(summary), encoding="utf-8")
        return summary
    ...
    hpo.run_hpo(
        results_root=tmp_path, base_config=base, base_seed=90000, n_cores=2,
        n_trials=1, sampler_seed=123, startup_trials=0,
        objective_mode="robust", study_name="resume_guard_v3",
    )
    with pytest.raises(RuntimeError, match="base_seed"):
        hpo.run_hpo(
            results_root=tmp_path, base_config=base, base_seed=91000, n_cores=2,
            n_trials=1, sampler_seed=123, startup_trials=0,
            objective_mode="robust", study_name="resume_guard_v3",
        )
